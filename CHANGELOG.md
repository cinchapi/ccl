...
* [GH-52](https://github.com/cinchapi/ccl/issues/52): Added the `strict(...)` condition wrapper. Tells the engine that all conditions inside the group must be satisfied by the **same** destination record, rather than being evaluated independently (which can produce false positives on multi-valued links). Supports [cinchapi/concourse#533](https://github.com/cinchapi/concourse/issues/533).
  * **In CCL**: wrap the conjunction with `strict(...)`, e.g. `find where strict(friend.name = "Jeff" AND friend.age > 30)`. Inputs without a shared navigation prefix degrade to normal `AND`/`OR` evaluation.
  * **In parsing code**: the parser produces a `StrictConditionTree` node wrapping the inner `ConditionTree`. `Visitor` implementations must explicitly handle `StrictConditionTree` — the default `visit(StrictConditionTree)` throws.
  * `strict` is now a reserved keyword and can no longer be used as a key name.
