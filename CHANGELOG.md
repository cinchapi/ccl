...
* [GH-52](https://github.com/cinchapi/ccl/issues/52): Added the `strict(...)` condition wrapper for same-destination navigation evaluation (e.g., `find where strict(friend.name = "Jeff" AND friend.age > 30)`), supporting [cinchapi/concourse#533](https://github.com/cinchapi/concourse/issues/533). The parser produces a new `StrictConditionTree` AST node that wraps any valid inner expression; the engine applies the same-destination constraint (or degrades to normal `AND`/`OR` when there is no shared navigation prefix). The `Visitor` interface has a new default `visit(StrictConditionTree)` method that transparently passes through to the wrapped condition, so existing visitors that do not care about strict semantics continue to behave as before; visitors that must honor strict semantics should override it. `strict` is now a reserved keyword and can no longer be used as a key name.
