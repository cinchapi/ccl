...
public class JavaCCParserLogicTest {
    ...
    public void testSingleNRegexExpressionTokenize() {
        ...
        expectedTokens.add(new OperatorSymbol(PARSER_TRANSFORM_OPERATOR_FUNCTION.apply("nregex")));
    ...
    public void testExplicitFunctionWithCCLAsEvaluationValue() {
        ...
        Assert.assertTrue(rootNode.parseTree() instanceof ExpressionTree);
        ExpressionSymbol expression = (ExpressionSymbol) rootNode.parseTree().root();
        Assert.assertEquals("age", expression.key().toString());
        Assert.assertEquals(">", expression.operator().toString());
    ...
    public void testOrderAbstractSyntaxTree() {
        ...
        Assert.assertTrue(rootNode.orderTree() != null);
        OrderTree orderTree = rootNode.orderTree();
    ...
    public void testMultipleOrderAbstractSyntaxTree() {
        ...
        Assert.assertEquals(2, ((OrderSymbol)orderTree.root()).order().spec().size());
        Assert.assertEquals("a", ((OrderSymbol)orderTree.root()).order().spec().get(0).key());
        Assert.assertEquals(Direction.ASCENDING, ((OrderSymbol)orderTree.root()).order().spec().get(0).direction());
        Assert.assertEquals("b", ((OrderSymbol)orderTree.root()).order().spec().get(1).key());
        Assert.assertEquals(Direction.DESCENDING, ((OrderSymbol)orderTree.root()).order().spec().get(1).direction());
    }
